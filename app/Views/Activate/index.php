...
<div class="row">
    <div class="col-sm-12 col-md-12">
        <div class="panel panel-default" style="{overflow-x: scroll;}"> 
            ...
            <table id = "activatetable" class="table table-bordered table-striped display table-datatable table table-bordered table-striped table-hover table-heading table-datatable">
              <thead>
                <tr>
                    <th>Id</th>
                    <th>Full Name</th>
...
<script type="text/javascript">
	...
	function getButton(data, type, dataToSet) {
		if(data.usergroup == 0){
    		return '<button pull-right" type="button" class="btn btn-warning">Activate</button>';
		}
		return '<button pull-right" type="button"  disabled class="btn btn-default">Activated</button>';
	}
    
    function formatDate(data, type, dataToSet)
    {
       var split =  data.createddate.split(" ");
        return split[0];
    ...
    $(document).ready(function() {
        var ptable = $('#activatetable').DataTable( {
            "processing": true,
            "serverSide": true,
            "ajax": "activateuser",
             "columnDefs": [
		  ...
		  $('#activatetable tbody').on('click', 'tr', function () {
            var transactionid = $('td', this).eq(0).text();
            var username = $('td', this).eq(2).text();
